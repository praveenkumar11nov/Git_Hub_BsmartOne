...
public abstract class GenericDAOImpl<T> implements GenericDAO<T> {
	...
	protected EntityManager customEntityManager=null;
	...
	@Override
	public T save(final T t,HttpServletRequest request) {		
		try {
			HttpSession session =request.getSession(false);
			  if(customEntityManager==null){
			   EntityManagerFactory emf = Persistence.createEntityManagerFactory((String)session.getAttribute("schemaName"));
			   customEntityManager= emf.createEntityManager();
			   }
			   customEntityManager.getTransaction().begin();
			   customEntityManager.persist(t);
	...
	@Override
	public void delete(final Object id,HttpServletRequest request) 
	{
		try {
			HttpSession session =request.getSession(false);
			  if(customEntityManager==null){
			   ...
			   customEntityManager.getTransaction().begin();
			   customEntityManager.remove(this.entityManager.getReference(type, id));
			   customEntityManager.getTransaction().commit();
		} catch (RuntimeException re) {
			throw re;
		}
	...
	@Override
	public T update(final T t,HttpServletRequest request) {
		...
		try {
			HttpSession session =request.getSession(false);
			  if(customEntityManager==null){
			   ...
			   customEntityManager.getTransaction().begin();
			   T result = customEntityManager.merge(t);
			   customEntityManager.getTransaction().commit();
			   return result;
		} catch (RuntimeException re) {
			throw re;
	...
	@Transactional(propagation = Propagation.SUPPORTS)
	@Override
	public T find(final Object id,HttpServletRequest request) {		
		try {
			HttpSession session =request.getSession(false);
			  if(customEntityManager==null){
			   EntityManagerFactory emf = Persistence.createEntityManagerFactory((String)session.getAttribute("schemaName"));
			...
			return (T) customEntityManager.find(type, id);
		} catch (RuntimeException re) {
			throw re;
		}
	}
